...
TEST_SPLIT = 0.2
SAVE_DIR = TEXT_DATA_DIR + '/model.model'
...
with open(os.path.join(GLOVE_DIR, 'wiki.zh.vec') ) as f: 
    header = f.readline()
...
for folderName in sorted(os.listdir(TEXT_DATA_DIR)):
    ...
    print('Info: path =', path, '-- label_id =', label_id)
...
print('Info: labels before categorical')
#print('	', labels)
labels = to_categorical(np.asarray(labels))
print('Info: labels after categorical')
#print('	', labels)
...
x_test = data[-(num_validation_samples+num_test_samples) : -num_validation_samples]
...
y_predicted = model.predict(x = x_test, batch_size=None, verbose=1, steps=None)
...
for i in range(0, len(x_test)):
    ...
    resultDict[tuple(y_test[i].tolist())] = resultDict.get(tuple(y_test[i].tolist()), [])
    resultDict[tuple(y_test[i].tolist())].append(y_predicted[i])
...
roc_y_true = []
roc_y_score = []
roc_pos_label = ''.join(map(str, (0., 1.)))
'''[key for key in resultDict][0]'''
print([key for key in resultDict][0])
...
for key in resultDict:
    ...
    for res in resultDict[key]:
        base += res
        roc_y_true.append(''.join(map(str, key)))
        roc_y_score.append(res[1])
...
fpr, tpr, thresholds = metrics.roc_curve(y_true = roc_y_true, y_score = roc_y_score, pos_label = roc_pos_label)
'''
print('fpr -- tpr')
